...
static void _cb(
   int pi, int gpio, unsigned level, uint32_t tick, void *user)
{
...
RED_t *RED(int pi, int gpioA, int gpioB, int mode, RED_CB_t cb_func)
{
   ...
   self->cb_id_a = callback_ex(pi, gpioA, EITHER_EDGE, (CBFuncEx_t) _cb, self);
   self->cb_id_b = callback_ex(pi, gpioB, EITHER_EDGE, (CBFuncEx_t) _cb, self);
